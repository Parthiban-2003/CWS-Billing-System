...
export default function Customers() {
    ...
    const dueOf = (id) =>
        invoices
            .filter(
                (v) => v.customerId === id && v.status !== 'CANCELLED'
            )
            ...
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                {customers.map((c) => {
                    ...
                    return (
                        <div
                            key={c.id}
                            className="bg-card border border-line rounded-xl p-4 hover:border-primary/50 transition"
                        >
                            {/* Customer details - click to open customer */}
                            <div
                                onClick={() => setSel(c)}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setEdit(c)}
                                        className="text-mut hover:text-primary p-1"
                                    >
                                        <Pencil size={14} />
                                    </button>
                                </div>
                                ...
                                <p
                                    className={
                                        due > 0
                                            ? 'text-rose-400 font-extrabold text-sm mt-2'
                                            : 'text-emerald-400 text-xs font-bold mt-2'
                                    }
                                >
