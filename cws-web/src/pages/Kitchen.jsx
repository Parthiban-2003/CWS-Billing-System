    ...
    return (
        <div className="min-h-screen bg-bg text-ink p-4">
            <header className="flex justify-between items-center mb-4">
                ...
                <p className="text-mut text-sm font-bold">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {COLS.map((c) => (
                    <div
                        key={c.id}
                        className="bg-card border border-line rounded-xl p-3 space-y-2 min-h-[70vh]"
                    >
                        <p className="font-extrabold text-sm text-mut">
                            {c.label} ({kots.filter((k) => k.status === c.id).length})
                        </p>

                        {kots
                            .filter((k) => k.status === c.id)
                            .map((k) => {
                                const wait = Math.floor(
                                    (now - new Date(k.createdAt)) / 60000
                                )
                                return (
                                    <div
                                        key={k.id}
                                    ...
                                    >
                                        <div className="flex justify-between font-extrabold text-sm">
                                            <span>#{k.number} · {k.table}</span>
                                            <span>{wait}m</span>
                                        </div>
                                        {k.items.map((i) => (
                                            <p key={i.id} className="text-sm">
                                                <b className="text-primary">{i.qty}×</b> {i.name}
                                            </p>
                                        ))}
                                        {c.btn && (
                                            <button
                                                onClick={() => move(k.id, c.next)}
