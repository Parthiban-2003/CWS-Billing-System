...
export default function PaymentModal({ open, onClose, totals }) {
    ...
    const recv =
        received === '' ? (method === 'CREDIT' ? 0 : finalTotal) : Number(received)
    const paid = split
        ? rows.reduce((s, r) => s + (Number(r.amount) || 0), 0)
        : Math.min(recv, finalTotal)
    const due = finalTotal - paid
    const change =
        !split && method === 'CASH' ? Math.max(0, recv - finalTotal) : 0

    // ⭐ LIVE POINTS PREVIEW (₹100 = 1 point)
    const earnedPreview =
        customer && settings?.loyaltyEnabled ? Math.floor(paid / 100) : 0
    const newBalance = maxPts + earnedPreview - redeemPts
    ...
    const confirm = async () => {
        ...
        if (split && Math.abs(paid - finalTotal) > 0.5) {
            toast.error(`Split total ${inr(paid)} ≠ ${inr(finalTotal)}!`)
            return
        }

        const { items, table, orderType, itemDiscount, serviceCharge } =
            useCartStore.getState()

        try {
            await api.post('/api/invoices', {
                items: items.map((i) => ({
                    id: i.isCombo ? null : i.id,
                    name: i.name,
                    qty: i.qty,
                    price: i.unitPrice,
                ...
                method,
                payments: split
                    ? rows.map((r) => ({ method: r.method, amount: Number(r.amount) || 0 }))
                    : [{ method, amount: paid }],
                paid,
    ...
    return (
        <Modal open={open} onClose={onClose} title="💳 Payment">
            <p className="text-center text-3xl font-extrabold text-primary mb-1">
                {inr(finalTotal)}
            </p>
            {redeemPts > 0 && (
                <p className="text-center text-xs font-bold text-emerald-400 mb-2">
                    ⭐ {redeemPts} points redeemed (−{inr(redeemPts)})
                </p>
            )}
            ...
            <div className="grid grid-cols-4 gap-2 mb-3">
                {METHODS.map(({ id, label, icon: Icon }) => (
                    <button
                        ...
                        className={cn(
                            'rounded-xl border p-2.5 flex flex-col items-center gap-1 text-[11px] font-bold transition',
                            method === id
            ...
            <div className="space-y-2 mb-3">
                {customer ? (
                    <div className="rounded-lg bg-primary-soft border border-primary/30 px-3 py-2">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-extrabold">👤 {customer.name}</p>
                                <p className="text-[10px] text-mut">
                                    {customer.phone || '—'} · ⭐ {maxPts} points available
                                </p>
                            </div>
                            <button
                                onClick={() => {
                                ...
                                className="text-mut hover:text-rose-400"
                            >
                                <X size={14} />
                            </button>
                        </div>

                        {/* ⭐ LIVE LOYALTY PREVIEW */}
                        {settings?.loyaltyEnabled && (
                            <div className="mt-2 rounded-md bg-card border border-line px-2.5 py-1.5 space-y-0.5">
                                <p className="text-[11px] font-bold text-mut flex justify-between">
                                    <span>Current points</span>
                                    <span className="text-amber-400">⭐ {maxPts}</span>
                                </p>
                                <p className="text-[11px] font-bold text-mut flex justify-between">
                                    <span>This bill earns</span>
                                    <span className="text-emerald-400">+{earnedPreview}</span>
                                </p>
                                {redeemPts > 0 && (
                                    <p className="text-[11px] font-bold text-mut flex justify-between">
                                        <span>Redeeming</span>
                                        <span className="text-rose-400">−{redeemPts}</span>
                                    </p>
                                )}
                                <p className="text-[11px] font-extrabold flex justify-between border-t border-line pt-1">
                        ...
                        {/* ⭐ REDEEM INPUT */}
                        {settings?.loyaltyEnabled && maxPts > 0 && (
                            <div className="mt-2">
                                <p className="text-[10px] font-bold text-mut mb-1">
                                    Points redeem pannu (1 pt = ₹1, max {maxPts})
                                </p>
                                <Input
                                    type="number"
                                    placeholder="0"
                    ...
                    <>
                        ...
                        {quickAdd && (
                            <div className="flex gap-2">
                                ...
                                />
                                <Button disabled={!qa.name} onClick={quickSave}>
                                    Add
                                </Button>
                            </div>
                        )}
                ...
                {!split && (
                    <>
                        <input
                            ...
                            placeholder={
                                method === 'CREDIT' ? '0 (full udhaar)' : String(finalTotal)
                            }
                ...
                {split && (
                    <div className="space-y-1.5">
                        ...
                        <button
                            ...
                            className="text-[11px] font-bold text-primary hover:underline"
                        >
                            <Plus size={12} className="inline" /> Add payment row
                        </button>
                        <p
                            className={cn(
                                'text-xs font-bold',
                                Math.abs(paid - finalTotal) < 0.5
                                    ? 'text-emerald-400'
                                    : 'text-rose-400'
                            )}
                        >
            ...
            <Button className="w-full mt-4" onClick={confirm}>
                Confirm Payment ✅
            </Button>
        </Modal>
    )
